...
static inline value Val_Display(Display *dpy) {
...
value Val_XScreen(Screen *s) { return caml_copy_nativeint((intnat)s); }

Screen *XScreen_val(value v) { return (Screen *)Nativeint_val(v); }
...
#define UInt_assert_val(x)                                                     \
  (Long_val(x) < 0 ? assert_wrong : UInt_noassert_val(x))

#define ULong_noassert_val(x) ((unsigned long)Long_val(x))
#define ULong_assert_val(x)                                                    \
  (Long_val(x) < 0 ? assert_wrong : ULong_noassert_val(x))
...
#define copy_obj(type, obj, ml_return)                                         \
  do {                                                                         \
    ...
    memcpy(Data_custom_val(ml_return), &obj, sizeof(type));                    \
  } while (0)
